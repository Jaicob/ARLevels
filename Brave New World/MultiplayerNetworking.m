...
{
    Message message;
    __unsafe_unretained   NSData *dataDictionary;
} MessageDictionary;
...
- (void)sendDictionary: (NSData *)dataDictionary{
    NSLog(@"Send Message Dictionary");
    MessageDictionary message;
    message.message.messageType = kMessageTypeDictionary;
    message.dataDictionary = dataDictionary;
    NSData *data = [NSData dataWithBytes:&message length:sizeof(MessageDictionary)];
    [self sendData:data];
...
- (void)match:(GKMatch *)match didReceiveData:(NSData *)data
   fromPlayer:(NSString *)playerID {
    ...
    }else if(message->messageType == kMessageTypeDictionary){
        NSLog(@"Message Dictionary Receieved");
        MessageDictionary * messageDictionary = (MessageDictionary *) [data bytes];
        NSLog(@"Dict:%@", [NSKeyedUnarchiver  unarchiveObjectWithData:messageDictionary->dataDictionary]);
        NSDictionary *dict = [NSKeyedUnarchiver  unarchiveObjectWithData:messageDictionary->dataDictionary];
        [self.delegate startLevelWithDict:dict];
    }else{
        NSLog(@"Dict:%@", [NSKeyedUnarchiver  unarchiveObjectWithData:data]);
