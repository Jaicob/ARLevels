    ...
    if (APPSTATUS_ERROR == appStatus) {
        ...
        switch (errorCode) {
            ...
            case QCAR_ERRCODE_NO_SERVICE_AVAILABLE:
                msg = msgNoService;
                break;
            case QCAR::INIT_NO_CAMERA_ACCESS:
                msg = msgNoCamera;
                break;
    ...
    if (QCAR::CameraDevice::getInstance().init(activeCamera)) {
        ...
        if (QCAR::CameraDevice::getInstance().start()) {
            ...
            QCAR::Tracker* tracker = trackerManager.getTracker(targetType == TYPE_FRAMEMARKERS ?
                                                               ...
                                                               QCAR::ObjectTracker::getClassType());
            if(tracker != 0)
                tracker->start();
...
- (QCAR::DataSet *)loadDataSet:(NSString *)dataSetPath
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (!theDataSet->load([dataSetPath cStringUsingEncoding:NSASCIIStringEncoding], QCAR::STORAGE_APPRESOURCE))
            {
                msg = msgFailedToLoad;
...
{
    QCAR::TrackerManager& trackerManager = QCAR::TrackerManager::getInstance();
    QCAR::ObjectTracker* objectTracker = (QCAR::ObjectTracker*)
